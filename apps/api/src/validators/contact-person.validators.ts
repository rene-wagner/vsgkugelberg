...
export const createContactPersonValidator = [
  ...
  body('address').optional({ values: 'null' }).trim().isLength({ max: 500 }).withMessage('Address must be at most 500 characters'),

  body('phone').trim().notEmpty().withMessage('Phone is required'),

  body('profileImageId').optional().isInt({ min: 1 }).withMessage('Profile image ID must be a positive integer'),
];
...
export const updateContactPersonValidator = [
  ...
  body('email').optional({ values: 'null' }).trim().isEmail().withMessage('Email must be a valid email address'),
...
export const idParamValidator = [param('id').isInt({ min: 1 }).withMessage('ID must be a positive integer')];
