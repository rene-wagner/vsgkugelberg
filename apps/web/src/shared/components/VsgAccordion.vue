...
<template>
  <div class="space-y-4">
    ...
    >
      <button class="w-full p-6 flex items-center justify-between text-left hover:bg-gray-50 transition-colors" @click="toggle(item.id)">
      ...
      >
        <div class="p-6 pt-0 font-body text-base text-gray-600 space-y-4 border-t border-gray-100 mt-4">
          <div v-for="(entry, index) in item.content" :key="index" class="flex gap-4">
            <span v-if="entry.year" class="text-vsg-gold-600 font-bold shrink-0">{{ entry.year }}</span>
            <p>{{ entry.text }}</p>
          </div>
        </div>
      </div>
