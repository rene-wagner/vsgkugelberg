...
const navGroups: NavGroup[] = [
  {
    ...
    items: [
      ...
      { name: 'Mediathek', path: '/admin/mediathek', icon: 'image' },
      { name: 'Benutzer', path: '/admin/benutzer', icon: 'users' },
      { name: 'Einstellungen', path: '/admin/einstellungen', icon: 'cog' },
    ],
  },
  {
    label: 'Seiten',
    items: [
      { name: 'Startseite', path: '/admin/home', icon: 'house' },
      {
        name: 'Verein',
        icon: 'people-group',
        children: [
  ...
  <aside class="fixed left-0 top-16 bottom-0 w-64 bg-vsg-blue-950 border-r border-vsg-gold-400/10 overflow-y-auto">
    <nav class="py-6">
      ...
      >
        ...
        >
          <span class="font-body font-normal text-xs tracking-widest text-vsg-blue-400 uppercase">{{ group.label }}</span>
        </div>
