...
async function handleSubmit() {
  ...
  try {
    ...
    if (result.success) {
      ...
      successMessage.value = result.message || 'Dein Passwort wurde erfolgreich geaendert.';
    } else {
...
<template>
  ...
  <div v-if="isSubmitted" class="text-center animate-slide-up">
    ...
    <RouterLink to="/login" class="inline-flex items-center gap-2 font-body font-normal text-vsg-gold-400 hover:text-vsg-gold-300 transition-colors">
      Zum Login
  ...
  <form v-else class="space-y-6" @submit.prevent="handleSubmit">
    ...
    <div class="animate-slide-up delay-300">
      <VsgPasswordInput id="password" v-model="password" label="Neues Passwort" required :minlength="8" variant="auth" />
      <p class="mt-1 text-xs text-vsg-blue-300">Min. 8 Zeichen, Gross-/Kleinbuchstaben und Zahl</p>
    </div>

    <!-- Confirm Password Input -->
    <div class="animate-slide-up delay-400">
      <VsgPasswordInput id="confirmPassword" v-model="confirmPassword" label="Passwort bestaetigen" required :minlength="8" variant="auth" />
    </div>
