...
<template>
  <section class="relative bg-white py-32">
    <div class="mx-auto max-w-7xl px-6">
      <VsgSectionHeader subtitle="Unsere Abteilungen" title="ABTEILUNGEN" class="mb-20" />
      ...
      <div v-else-if="error" class="mx-auto max-w-2xl rounded-lg bg-red-50 p-6 text-center">
        ...
        <button class="mt-4 rounded bg-vsg-blue-600 px-4 py-2 text-white hover:bg-vsg-blue-700" @click="departmentsStore.fetchDepartments()">
          Erneut versuchen
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="departments.length === 0" class="mx-auto max-w-2xl rounded-lg bg-gray-50 p-12 text-center">
