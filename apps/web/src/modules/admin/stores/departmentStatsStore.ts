...
export const useDepartmentStatsStore = defineStore('departmentStats', () => {
  ...
  async function createStat(
    slug: string,
    data: CreateDepartmentStatDto,
  ): Promise<DepartmentStat | null> {
    isLoading.value = true;
    error.value = null;

    try {
      const response = await fetch(
        `${API_BASE_URL}/api/departments/${slug}/stats`,
        {
          method: 'POST',
          headers: {
  ...
  async function updateStat(
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/departments/${slug}/stats/${id}`,
        {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
  ...
  async function reorder(
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/departments/${slug}/stats/reorder`,
        {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
