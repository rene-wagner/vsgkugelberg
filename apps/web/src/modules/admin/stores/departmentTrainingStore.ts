...
export const useDepartmentTrainingStore = defineStore('departmentTraining', () => {
  ...
  async function createGroup(slug: string, data: CreateDepartmentTrainingGroupDto): Promise<DepartmentTrainingGroup | null> {
    isLoading.value = true;
    error.value = null;

    try {
      const response = await fetch(`${API_BASE_URL}/api/departments/${slug}/training-groups`, {
        method: 'POST',
        headers: {
  ...
  async function updateGroup(slug: string, groupId: number, data: UpdateDepartmentTrainingGroupDto): Promise<DepartmentTrainingGroup | null> {
    isLoading.value = true;
    error.value = null;

    try {
      const response = await fetch(`${API_BASE_URL}/api/departments/${slug}/training-groups/${groupId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async function deleteGroup(slug: string, groupId: number): Promise<boolean> {
    isLoading.value = true;
    error.value = null;
    ...
    try {
      ...
      trainingGroups.value = trainingGroups.value.filter((g) => g.id !== groupId);
      return true;
    } catch (e) {
      error.value = e instanceof Error ? e.message : 'An error occurred';
      return false;
  ...
  async function reorderGroups(slug: string, ids: number[]): Promise<DepartmentTrainingGroup[] | null> {
    isLoading.value = true;
    error.value = null;

    try {
      const response = await fetch(`${API_BASE_URL}/api/departments/${slug}/training-groups/reorder`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const reorderedGroups = (await response.json()) as DepartmentTrainingGroup[];
  ...
  async function createSession(slug: string, groupId: number, data: CreateDepartmentTrainingSessionDto): Promise<DepartmentTrainingSession | null> {
    isLoading.value = true;
    error.value = null;

    try {
      const response = await fetch(`${API_BASE_URL}/api/departments/${slug}/training-groups/${groupId}/sessions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async function updateSession(
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/departments/${slug}/training-groups/${groupId}/sessions/${sessionId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const updatedSession = (await response.json()) as DepartmentTrainingSession;
      const group = trainingGroups.value.find((g) => g.id === groupId);
      if (group) {
  ...
  async function deleteSession(slug: string, groupId: number, sessionId: number): Promise<boolean> {
    isLoading.value = true;
    error.value = null;
  ...
  async function reorderSessions(slug: string, groupId: number, ids: number[]): Promise<DepartmentTrainingSession[] | null> {
    isLoading.value = true;
    error.value = null;

    try {
      const response = await fetch(`${API_BASE_URL}/api/departments/${slug}/training-groups/${groupId}/sessions/reorder`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const reorderedSessions = (await response.json()) as DepartmentTrainingSession[];
      const group = trainingGroups.value.find((g) => g.id === groupId);
      if (group) {
