...
export const useDepartmentLocationsStore = defineStore(
  'departmentLocations',
  () => {
    ...
    async function createLocation(
      slug: string,
      data: CreateDepartmentLocationDto,
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/departments/${slug}/locations`,
          {
            method: 'POST',
            headers: {
    ...
    async function updateLocation(
      slug: string,
      id: number,
      data: UpdateDepartmentLocationDto,
    ): Promise<DepartmentLocation | null> {
      isLoading.value = true;
      error.value = null;
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/departments/${slug}/locations/${id}`,
          {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
    ...
    async function reorder(
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/departments/${slug}/locations/reorder`,
          {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
        ...
        const reorderedLocations =
          (await response.json()) as DepartmentLocation[];
